    ...
    <header className="w-full sticky top-0 z-20 bg-black/40 backdrop-blur border-b border-white/10">
      <div className="max-w-6xl mx-auto px-4 py-3 flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          <a
            href="/origins"
            className="rounded-xl px-2 py-1 text-xs font-semibold uppercase tracking-wider text-white/70 hover:text-[#CBFF00]"
          >
            {lang === "es" ? "Origen" : "Origins"}
          </a>
  ...
  return (
    <section className="relative max-w-6xl mx-auto px-4 pt-12 md:pt-16 pb-12">
      ...
      <h1 className="text-5xl md:text-7xl font-extrabold tracking-tight leading-tight">
        {lang === "es" ? (
          <>
            1,1 MILLONES DE DOTS. <span className="text-[#CBFF00]">UNA CADENA POR INVADIR.</span>
          </>
        ) : (
          <>
            1.1 MILLION DOTS. <span className="text-[#CBFF00]">ONE CHAIN TO INVADE.</span>
          </>
        )}
      </h1>
      ...
      <p className="mt-4 text-white/80 max-w-3xl text-xl md:text-2xl">
        {lang === "es" ? (
          <>
            <strong>Tan pequeños que puedes ignorarlos. Demasiados para detenerlos.</strong>
            <br />
            1.100.000 personajes generativos. Mint aleatorio. Solo 1 USD en{" "}
            <strong className="text-[#CBFF00]">Robinhood Chain</strong>.
          </>
        ) : (
          <>
            <strong>Small enough to ignore. Too many to stop.</strong>
            <br />
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-3">
        {items.map((it, idx) => (
          <div key={`fact-${idx}`} className="rounded-2xl border border-white/10 bg-white/5 p-4 text-center">
            ...
            <div className={`mt-1 text-3xl md:text-4xl font-bold ${it.v === "Robinhood" ? "text-[#CBFF00]" : ""}`}>{it.v}</div>
          </div>
        ))}
      </div>
    </section>
  ...
  return (
    <footer className="border-t border-white/10 text-center text-xs text-white/60 py-8">
      <div className="max-w-6xl mx-auto px-4 flex flex-col sm:flex-row items-center justify-center gap-3">
        <span>
          © {new Date().getFullYear()} DOTS — Built on <span className="text-[#CBFF00]">Robinhood Chain</span>. All rights reserved.
        </span>
        <a href="/origins" className="text-white/70 hover:text-[#CBFF00]">
          Origins / Origen
        </a>
      </div>
    </footer>
