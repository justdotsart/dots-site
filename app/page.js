      ...
      <div className="max-w-6xl mx-auto px-4 py-3 flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          <a
            ...
            className="rounded-2xl px-3 py-1.5 bg-white text-black text-sm font-semibold hover:bg-[#CBFF00]"
  ...
  return (
    <div className="flex items-center gap-2 min-h-8">
      {!failed ? (
        ...
        <>
          <div className="w-3 h-3 rounded-full bg-[#CBFF00] animate-pulse" />
...
function Hero({ lang }) {
  return (
    <section className="relative max-w-6xl mx-auto px-4 pt-12 md:pt-16 pb-12">
      <div
        className="flex justify-end items-center gap-2 mb-10 -mt-12"
        aria-hidden
      >
        {TOP_DOT_IDS.map((i) => (
          <DotImage
      ...
      <p className="mt-4 text-white/80 max-w-3xl text-xl md:text-2xl">
        {lang === "es" ? (
          ...
          <>
            <strong>
              Small enough to ignore. Too many to stop.
            </strong>
      ...
      <div className="mt-6 flex flex-wrap gap-3">
        <a
          ...
          target="_blank"
          rel="noopener noreferrer"
          className="rounded-2xl px-5 py-3 bg-white text-black font-semibold hover:bg-[#CBFF00]"
          aria-label="Mint DOTS"
        >
          Mint DOTS
        </a>
...
function KeyFacts({ lang }) {
  const items = useMemo(
    () => [
      ...
      {
        k: lang === "es" ? "Seres" : "Types",
        v: "10",
      },
      {
        k: lang === "es" ? "Rasgos" : "Traits",
        v: lang === "es" ? "Muchos" : "A lot",
      },
      {
        k: lang === "es" ? "Red" : "Network",
        v: "Robinhood",
      },
      {
        k: lang === "es" ? "Precio" : "Price",
        v: "$1",
      },
    ],
    [lang]
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-3">
        {items.map((item, index) => (
          ...
          >
            ...
            <div
              className={`mt-1 text-3xl md:text-4xl font-bold ${
                item.v === "Robinhood" ? "text-[#CBFF00]" : ""
              }`}
            >
              {item.v}
            </div>
    ...
    <img
      ...
      onError={() => {
        if (currentSrc.endsWith(".png")) {
          setCurrentSrc(currentSrc.replace(/\.png$/i, ".PNG"));
        } else if (currentSrc.endsWith(".PNG")) {
          setCurrentSrc(currentSrc.replace(/\.PNG$/i, ".webp"));
        } else {
          setOk(false);
        }
...
function MiniGallery({ lang }) {
  const baseItems = DOTS_THUMBS.length
    ? DOTS_THUMBS
    : PLACEHOLDER_DOTS;
  ...
  const computeStep = () => {
    ...
    const gapElement =
      trackRef.current || list.querySelector("[data-track]");

    const style = gapElement
      ? window.getComputedStyle(gapElement)
      : window.getComputedStyle(list);

    const gap = parseFloat(
      style.columnGap || style.gap || "12"
    );

    return (
      (firstCard?.getBoundingClientRect().width || cardW) +
      gap
    );
  };
    ...
    >
      <div className="flex items-end justify-between mb-4">
        ...
        <div className="text-sm text-white/70">
          {lang === "es"
            ? "Auto-carrusel • orden aleatorio"
            : "Auto-carousel • random order"}
        </div>
      </div>
          ...
          >
            {loopItems.map((item, index) => (
              <div
                ...
                style={{
                  width: NATIVE_W * SCALE + 16,
                  height: NATIVE_H * SCALE + 16,
                }}
              >
                {typeof item === "string" ? (
                  <Thumb
                    src={item}
                    alt={`DOT #${(index % baseLen) + 1}`}
...
function PixelDisc({ hue, seed }) {
  ...
  useEffect(() => {
    ...
    const random = (number) =>
      Math.abs(Math.sin(number * 9999 * seed));
    ...
    for (let y = 0; y < size; y++) {
      for (let x = 0; x < size; x++) {
        ...
        if (distance > size * 0.48) continue;

        const tone = random(x * 0.13 + y * 0.17);
        const lightness = 45 + Math.floor(tone * 45);
    ...
    <section className="max-w-6xl mx-auto px-4 pb-20">
      <div className="rounded-3xl border border-white/10 bg-gradient-to-br from-white/10 to-white/5 p-6 md:p-10 text-center">
        <h3 className="text-3xl md:text-4xl font-bold">
          {lang === "es"
            ? "Únete a la órbita DOTS"
            : "Join the DOTS orbit"}
        </h3>

        <p className="text-white/80 mt-3 text-lg md:text-2xl">
          {lang === "es"
            ? "El mint está activo. Obtén un DOT aleatorio por 1 USD."
            : "The mint is live. Get a random DOT for $1."}{" "}
        ...
        <div className="mt-5 flex flex-wrap justify-center gap-3">
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="rounded-2xl px-5 py-3 bg-[#CBFF00] text-black font-semibold hover:bg-white"
          ...
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="rounded-2xl px-5 py-3 bg-white text-black font-semibold hover:bg-[#CBFF00] flex items-center gap-2"
          ...
          >
            ...
            >
              ...
              <path
                d="M178.2 70.7c-1.8 8.4-8.4 39.6-11.4 53.1-2.4 11.1-7.8 15.6-14.4 15.3-7.2-.3-10.8-4.8-19.2-9.3-17.4-9.6-28.8-15.3-41.4-23.4-8.1-5.1-7.8-8.1 1.8-12 3.9-1.8 16.2-5.4 26.1-8.1 9.9-2.7 26.1-7.2 38.4-11.4 2.7-1 5.4-.6 7.8 2.1 2.4 2.7 1.8 5.4.3 9.0z"
                fill="#fff"
              />
            </svg>
  ...
  return (
    <footer className="border-t border-white/10 text-center text-xs text-white/60 py-8">
      <div className="max-w-6xl mx-auto px-4">
        © {new Date().getFullYear()} DOTS — Built on{" "}
        <span className="text-[#CBFF00]">
          Robinhood Chain
        </span>
        . All rights reserved.
